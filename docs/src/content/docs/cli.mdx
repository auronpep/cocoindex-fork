...
* `-e, --env-file <path>`: Load environment variables from a specified `.env` file. If not provided, the CLI searches upward from the current directory for a `.env` file.
